...
for iteration in range(16):
    ...
    #                 if ls[u] < ellmin_gg:
        ...
        npar = 2*N+9
        for i in range(npar):
            vec1 = np.zeros(N+1+N+Ngg)
            if i < N:
                vec1[0] = 2.*cl_kki[i+skip][u]
                vec1[i+1] = 2.*cl_kg[i+skip][u]
                if ndensity[i] != 0:
                    vec1[i+N+1] = 2.*( cl_gg[i+skip][u] - 1./(ndensity[i+skip]/(8.462*10**-8)) )
                # else:
                #     vec1[i+N+1-skip] = 0
                # if ls[u] < ellmin_gg:
                #     vec1[i+N+1] = 0
                ...
                vec1[0] = 0
                vec1[i+1-N] = cl_kg[i-N+skip][u]
                if ndensity[i-N] != 0:
                    vec1[i+N+1-N] = 2.*( cl_gg[i-N+skip][u] - 1./(ndensity[i-N+skip]/(8.462*10**-8)) )
                # else:
                #     vec1[i+N+1-Ngen-skip] = 0
                # if ls[u] < ellmin_gg:
                #     vec1[i+N+1-N] = 0
            else:
                vec1[0] = dClkk_dpar[i-2*N][u]
                for jj in range(N):
                    vec1[jj+1] = dClkg_dpar[i-2*N][jj+skip][u]
                for jj in range(N):
                    vec1[1+N+jj] = dClgg_dpar[i-2*N][jj+skip][u]

            for j in range(npar):
                vec2 = np.zeros(N+1+N+Ngg)
                if j < N:
                    vec2[0] = 2.*cl_kki[j+skip][u]
                    vec2[j+1] = 2.*cl_kg[j+skip][u]
                    if ndensity[j] != 0:
                        vec2[j+N+1] = 2.*( cl_gg[j+skip][u] - 1./(ndensity[j+skip]/(8.462*10**-8)) )
                    # else:
                    #     vec2[j+N+1-skip] = 0
                    # if ls[u] < ellmin_gg:
                    #     vec2[j+N+1] = 0
                    ...
                    vec2[0] = 0
                    vec2[j+1-N] = cl_kg[j-N+skip][u]
                    if ndensity[j-Ngen] != 0:
                        vec2[j+N+1-N] = 2.*( cl_gg[j-N+skip][u] - 1./(ndensity[j-N+skip]/(8.462*10**-8)) )
                    # else:
                    #     vec2[j+N+1-Ngen-skip] = 0
                    # if ls[u] < ellmin_gg:
                    #     vec2[j+N+1-N] = 0
                else:
                    vec2[0] = dClkk_dpar[j-2*N][u]
                    for jj in range(N):
                        vec2[jj+1] = dClkg_dpar[j-2*N][jj+skip][u]
                    for jj in range(N):
                        vec2[1+N+jj] = dClgg_dpar[j-2*N][jj+skip][u]
                # if i ==0:
    ...
    # DataOut = np.column_stack((FisherM))
    # np.savetxt('Fisher_ppf_N16.dat', DataOut)
